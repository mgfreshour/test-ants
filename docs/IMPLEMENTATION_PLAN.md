...
10 two-week sprints. Each sprint produces a runnable build with visible new functionality. The plan front-loads core simulation systems (pheromones, ant AI, rendering) so that every subsequent sprint layers on top of a working, watchable ant colony.
...
Sprint  5  ██████████░░░░░░░░░░  Player Control
Sprint  6  ████████████░░░░░░░░  Combat & Enemies
Sprint  7  ██████████████░░░░░░  Environment & Hazards
Sprint  8  ████████████████░░░░  Quick Game Complete
Sprint  9  ██████████████████░░  Campaign Mode
Sprint 10  ████████████████████  Polish & Sandbox
```
...
## Sprint 5: Player Control (Weeks 9-10)
...
| # | Task | Components / Systems | Est |
|---|---|---|---|
| 5.1 | `PlayerControlled` marker component — exactly one ant has this at any time. Yellow sprite tint. Camera follows this ant optionally (toggle with `F` key). | `components/ant.rs` | 3h |
| 5.2 | Player movement — WASD overrides AI movement for the controlled ant. Collision with terrain obstacles. | `plugins/player.rs` | 4h |
| 5.3 | Player item interaction — E to pick up food/pebble, Q to drop. Carry indicator visible. | `plugins/player.rs` | 3h |
| 5.4 | Player pheromone laying — hold Shift while moving to deposit trail pheromone (yellow). Other ants follow this trail. | `plugins/player.rs` | 3h |
| 5.5 | Player attack — Space key attacks adjacent enemy. Attack animation/flash. | `plugins/player.rs` | 3h |
| 5.6 | Recruit system — R key: nearby friendly ants within 5-tile radius enter FOLLOW state, trail behind player. T key: dismiss all followers back to IDLE. | `plugins/player.rs`, `plugins/ant_ai.rs` | 5h |
| 5.7 | Exchange ant — Tab now opens ant-type selector (worker/soldier). Nearest ant of selected type becomes the new player ant. Previous ant resumes AI. | `plugins/player.rs` | 4h |
| 5.8 | Regurgitate food — F key: share food with adjacent nestmate (transfers carried food to another ant's hunger). | `plugins/player.rs` | 2h |
| 5.9 | Player HUD additions — controlled ant's HP bar, hunger bar, carried item indicator, follower count. | `ui/hud.rs` | 3h |
| 5.10 | Camera follow mode — when enabled, camera smoothly tracks player ant. Player can still nudge pan. Disable to free-cam. | `plugins/camera.rs` | 3h |
| 5.11 | Visual feedback — ant highlight glow, pheromone deposit particles, recruit radius indicator on R press. | rendering | 4h |
...
## Sprint 6: Combat & Enemies (Weeks 11-12)
...
| # | Task | Components / Systems | Est |
|---|---|---|---|
| 6.1 | Red colony — spawn a second colony entity with its own nest entrance, queen, starting ants (30). Red-tinted sprites. | `plugins/colony.rs` | 4h |
| 6.2 | Colony membership — `ColonyMember { colony_id }` component on every ant. Ants distinguish friend from foe by colony ID. | `components/ant.rs` | 2h |
| 6.3 | `CombatDetectionSystem` — when ants from different colonies occupy adjacent cells, trigger combat state | `plugins/combat.rs` | 4h |
| 6.4 | `CombatResolutionSystem` — per-tick damage calculation: base attack ± random ± group bonus ± terrain bonus. Apply damage to `Health`. | `plugins/combat.rs` | 5h |
| 6.5 | `DeathSystem` — ant at 0 HP despawns, drops carried item, emits `AntDiedEvent`. Optional: corpse sprite lingers briefly. | `plugins/combat.rs` | 3h |
| 6.6 | Alarm pheromone — fighting ants emit alarm pheromone. `AlarmResponseSystem` causes nearby DEFEND-role ants to switch to FIGHT and move toward source. | `plugins/pheromone.rs`, `plugins/ant_ai.rs` | 5h |
| 6.7 | Spider predator — entity with web zone. `SpiderAISystem`: wait at web, attack ants entering 3-tile radius, relocate after timeout. High HP, lethal to lone ants. | `plugins/predators.rs` | 5h |
| 6.8 | Antlion predator — pit trap entity at sandy areas. Ants entering pit slide to center, take damage. Antlion emerges to finish. | `plugins/predators.rs` | 4h |
| 6.9 | Red colony basic AI — same ant FSM but with a strategy layer that adjusts behavior sliders over time (start defensive, ramp to aggressive). | `plugins/ant_ai.rs` | 5h |
| 6.10 | Combat visual effects — hit flash, small damage numbers or shake, death particle burst. | rendering | 3h |
| 6.11 | Victory/defeat detection — `VictoryCheckSystem` monitors queen HP for both colonies. Queen death → colony collapse event. Game over screen. | `plugins/simulation.rs`, `ui/` | 3h |
...
## Sprint 7: Environment & Hazards (Weeks 13-14)
...
| # | Task | Components / Systems | Est |
|---|---|---|---|
| 7.1 | Rain event system — periodic rain (random interval 2-5 min). During rain: pheromone evaporation rate 10x, visual rain particle overlay, puddles on terrain. | `plugins/environment.rs` | 5h |
| 7.2 | Nest flooding — heavy rain causes water level to rise in lowest nest tunnels. Ants in flooded areas take damage. Player incentive to not dig too deep. | `plugins/environment.rs`, `plugins/nest.rs` | 4h |
| 7.3 | Human footstep event — random position, 3-tile radius area damage (999). Shadow warning 1 second before impact. | `plugins/environment.rs` | 3h |
| 7.4 | Lawnmower event — horizontal sweep from left edge to right at a random y-position. Kills everything in path. Visual/audio warning. | `plugins/environment.rs` | 4h |
| 7.5 | Pesticide spray — lingering poison zone (10x10 area). Ants entering take damage over time. Persists for 30 seconds. Visual green haze. | `plugins/environment.rs` | 3h |
| 7.6 | Day/night cycle — 3-minute cycle. Ants forage more during day, return to nest at night. Subtle lighting shift on terrain. | `plugins/environment.rs` | 4h |
| 7.7 | Weather visual effects — rain particles, darkened sky, puddle sprites, footstep shadow, mower sprite. | rendering | 5h |
| 7.8 | Sound effects (first pass) — ambient outdoor sounds, rain, combat clicks, food pickup chime, alarm tone. Use `bevy_audio`. | audio | 5h |
| 7.9 | Event notification system — brief text pop-ups: "Rain starting!", "Watch out — footstep!", "Lawnmower approaching!" | `ui/notifications.rs` | 3h |
...
## Sprint 8: Quick Game Complete (Weeks 15-16)
...
| # | Task | Components / Systems | Est |
|---|---|---|---|
| 8.1 | Main menu — title screen with "Quick Game", "Campaign" (grayed out), "Sandbox" (grayed out) buttons. | `ui/main_menu.rs` | 4h |
| 8.2 | Game state machine — Loading → MainMenu → InGame → Paused → GameOver. Bevy `States` with enter/exit systems. | `plugins/simulation.rs` | 4h |
| 8.3 | Pause menu — ESC pauses sim, shows Resume / Restart / Quit to Menu. | `ui/pause_menu.rs` | 3h |
| 8.4 | Game over screen — Victory: "Colony Dominant!" with stats (ants produced, food gathered, time). Defeat: "Colony Lost" with cause. | `ui/gameover.rs` | 3h |
| 8.5 | Quick game setup — balanced starting conditions. Black and red colony start equidistant. 5 food sources. 2 spiders. 1 antlion. | `plugins/simulation.rs` | 3h |
| 8.6 | Balance tuning — playtest and adjust: ant stats, food spawn rates, red AI aggression curve, predator HP, event frequency. Target: competitive game winnable in 10-15 min. | all systems | 8h |
| 8.7 | Red AI improvements — smarter raid timing, retreat behavior, expansion priority. Should feel like a competent opponent. | `plugins/ant_ai.rs` | 6h |
| 8.8 | Minimap — corner overlay showing full patch, ant density (black/red dots), food sources, predator locations. | `ui/minimap.rs` | 5h |
| 8.9 | Tutorial hints — first-time popups: "Press WASD to move", "Hold Shift to lay trail", "Press R to recruit", etc. Dismissable, non-intrusive. | `ui/tutorial.rs` | 4h |
| 8.10 | End-to-end QA — test full game loop 10+ times. Fix crashes, softlocks, degenerate strategies. | QA | 6h |
...
## Sprint 9: Campaign Mode (Weeks 17-18)
...
| # | Task | Components / Systems | Est |
|---|---|---|---|
| 9.1 | Campaign map screen — 4x4 grid of yard patches + house. Player selects which patch to enter. Colonized patches shown in black, red in red, neutral in gray. | `ui/campaign_map.rs` | 5h |
| 9.2 | Patch generation — each patch has procedurally varied terrain, food sources, hazards based on difficulty tier. | `plugins/campaign.rs`, `plugins/terrain.rs` | 6h |
| 9.3 | Mating flight mechanic — when colony hits critical population, player can trigger mating flight. Choose target adjacent patch. New colony established with starter queen + 10 workers. | `plugins/campaign.rs` | 5h |
| 9.4 | Patch switching — player can switch active patch from campaign map. Each patch maintains its own simulation state (ants, pheromones, nest). | `plugins/campaign.rs` | 6h |
| 9.5 | Difficulty scaling — later patches have more red ants, more predators, more hazards. House patches have poison bait, traps, exterminator events. | `plugins/campaign.rs` | 4h |
| 9.6 | Red colony campaign AI — red colony also expands to adjacent patches. Race for territory. Red AI more aggressive in later patches. | `plugins/ant_ai.rs` | 5h |
| 9.7 | House interior tileset — indoor tiles (kitchen tile, carpet, wood), indoor food sources (sugar bowl, crumbs under table, pet food). | `assets/`, `plugins/terrain.rs` | 5h |
| 9.8 | Campaign victory/defeat — win at 70% house + red eliminated. Lose if all queens dead. Stats screen with campaign summary. | `plugins/campaign.rs`, `ui/gameover.rs` | 3h |
| 9.9 | Campaign save/load — serialize campaign state (patch statuses, colony states) to disk. Resume from main menu. | `plugins/campaign.rs` | 5h |
...
## Sprint 10: Polish & Sandbox (Weeks 19-20)
...
| # | Task | Components / Systems | Est |
|---|---|---|---|
| 10.1 | Sandbox mode — spawn from menu. Full controls: spawn food, place walls, paint pheromones, spawn/kill ants, control either colony. | `plugins/sandbox.rs` | 6h |
| 10.2 | Sandbox parameter panel — expose all simulation constants (evap rate, diffuse rate, ant speed, lifespans, etc.) with live-edit sliders. | `ui/sandbox_panel.rs` | 4h |
| 10.3 | Data overlays for sandbox — pheromone heat maps, ant density map, population over time graph, foraging efficiency metric. | `ui/overlays.rs` | 5h |
| 10.4 | Art polish — replace placeholder sprites with proper pixel art. Ant walk animations (6-8 frames). Food sprites. Terrain tiles. UI skinning. | art, `assets/` | 8h |
| 10.5 | Audio polish — full ambient soundscape (surface/underground). Combat sounds. UI sounds. Dynamic music that responds to colony stress. | audio, `assets/audio/` | 6h |
| 10.6 | Performance optimization — profile with 10K ants. Optimize pheromone grid (SIMD or compute shader). Reduce AI update frequency. LOD for distant ants. Sprite batching audit. | all systems | 8h |
| 10.7 | Settings screen — resolution, fullscreen, volume sliders, control rebinding. | `ui/settings.rs` | 4h |
| 10.8 | Accessibility — colorblind mode for pheromone overlays, adjustable UI scale, key rebinding. | `ui/settings.rs` | 3h |
| 10.9 | Bug fix buffer — address bugs found in Sprint 8-9 playtesting. | all | 6h |
| 10.10 | Final QA — full pass on all three modes (Quick, Campaign, Sandbox). Performance benchmarks on min-spec hardware. | QA | 5h |
...
Sprint 1 ──► Sprint 2 ──► Sprint 3 ──► Sprint 4 ──┐
  (ants)     (pheromone)   (foraging)   (colony)    │
                                                     ...
                                                     │                                    (campaign)   (polish)
                                                     │
                                                     └─ Sprints 1-4 build the simulation engine.
                                                        Sprints 5-8 add gameplay on top.
                                                        Sprints 9-10 add breadth and polish.
```
